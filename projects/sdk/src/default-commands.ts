...
/**
 * Frozen command information copied for a matched message and passed to its guard and rejection callback.
...
> extends PrefixCommandDefinition {
    ...
    /**
     * Provide application feedback after a false guard, rejected arguments or a denied cooldown claim.
     * Receives raw context and a safe rejection classification, not partially converted values.
     * Return normally or await the work in a promise. Throws and rejected promises use attachment error reporting, without retry
     */
    readonly onReject?: (
...
export interface DefaultPrefixCommandRouter<M extends MessageCore = Message> {
     ...
     * Selecting a group by its registered names shows that group and its immediate children, including empty groups, unless an ancestor is hidden.
