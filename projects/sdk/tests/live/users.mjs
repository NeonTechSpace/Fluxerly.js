...
try {
    ...
    WebSocket.prototype.send = function (data, ...args) {
        try {
            ...
            if (ownedSockets.has(this) && payload.op === 3)
                presenceWrites.push({ socket: this, status: payload.d?.status, text: payload.d?.custom_status?.text })
        } catch {}
    ...
    assert.equal(
        history.data.filter((message) => message.author?.id === botId && message.content === `${journal.marker}:lost`)
            .length,
        1,
    )
    report(stage)
    ...
    if (!groupId || process.argv.includes("--without-group")) {
        console.log(
            JSON.stringify({ mode, check: "existing_group", skipped: true, reason: "No authorized group selected" }),
        )
    } else {
        assert.match(groupId, /^[1-9][0-9]{0,19}$/)
