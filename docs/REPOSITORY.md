...
| `test:live:rate-limits` | Built default/native cross-route waiting after an injected global 429, followed by live read recovery | Verifies sandbox identity and uses the shared lock. One synthetic rejection per API, followed by read-only bot-self and guild requests. No deliberate hosted throttling or remote mutation |
